...
from datetime import datetime, timedelta
plt.rcParams['font.family'] = 'Times New Roman'
file_path = '/Users/pjl/PycharmProjects/tailor project/R1_NS_Z8(WA5)_20220927_1100_1500_All_Matches.csv'
...
x=np.array(result_df['h2'])
y=np.array(result_df['l2']/0.613)

x1=np.array(result_df1['h2'])
y1=np.array(result_df1['l2']/0.390)

x2=np.array(result_df2['h2'])
y2=np.array(result_df2['l2']/0.542)

x3=np.array(result_df3['h2'])
y3=np.array(result_df3['l2']/0.367)

x4=np.array(result_df4['h2'])
y4=np.array(result_df4['l2']/0.574)

x5=np.array(result_df5['h2'])
y5=np.array(result_df5['l2']/0.306)
...
with pm.Model() as model:
    ...
    H_z6 = pm.Normal("H_z6",mu=4, sd=1.5)

    # H_z6 = pm.Normal("H_z6",mu=4, sd=2)
...
with pm.Model() as model:
    ...
    # MCMC sampling
    trace_z5 = pm.sample(2000, tune=1000, cores=1, random_seed=42)
...
# 保存到 CSV 文件
# df_combined.to_csv('combined_summaryintra1112.csv', index=False)
...
ax1.set_ylabel('出口路段换道系数',fontproperties=font_prop,fontsize=15)
ax1.set_xlim([1,6])
# 添加 x 轴标签
ax1.set_xlabel('车队内平均车头时距/(s)',fontproperties=font_prop,fontsize=15)
legend1 = ax1.legend(loc='upper right',prop=font_prop,fontsize=15)
...
ax2.set_ylabel('入口路段换道系数',fontproperties=font_prop,fontsize=15)
# 移动次坐标轴的左侧轴线到下方
ax2.set_xlabel('车队内平均车头时距/(s)',fontproperties=font_prop,fontsize=15)
...
output_path1 = f"{'/Users/pjl/Desktop/中国公路学报'}/{'ind567.png'}"
# 保存图表为高质量图片
fig1.savefig(output_path1, dpi=600, bbox_inches='tight')
output_path2 = f"{'/Users/pjl/Desktop/中国公路学报'}/{'ind234.png'}"
# 保存图表为高质量图片
...
# output_path2 = f"{'/Users/pjl/Desktop/中国公路学报'}/{'figindω.png'}"
# # 保存图表为高质量图片
